...
const CustomCheckbox: React.FC<CustomCheckboxProps> = ({ label, isChecked, onChange }) => {
  ...
  return (
    <div>
      ...
      <label
        htmlFor={id}
        className={`flex items-center p-3 rounded-xl transition-all duration-300 cursor-pointer border-2 bg-black/30 hover:bg-slate-700/50 shadow-md peer-focus-visible:outline peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-amber-400 ${isChecked ? 'bg-amber-300/10 border-amber-400' : 'border-slate-700'}`}
      >
