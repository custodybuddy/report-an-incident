...
const generateReportHTML = (reportData: ReportData, incidentData: IncidentData): string => {
    ...
    const sourcesList = (reportData.sources?.length > 0 ? reportData.sources : []).map(s => `<li><a href="${s}" target="_blank" rel="noopener noreferrer">${s}</a></li>`).join('');
    
    const evidenceItems = incidentData.evidence.length > 0 ? incidentData.evidence.map(e => `
        <div class="evidence-item">
            <div class="evidence-header">
                <strong>File:</strong> ${e.name}
            </div>
            <div class="evidence-details">
                <p><strong>Category:</strong> ${e.category}</p>
                <p><strong>Description:</strong> ${e.description || 'No description provided.'}</p>
                ${e.aiAnalysis ? `<p class="ai-analysis"><strong>AI Analysis:</strong> <span>${e.aiAnalysis}</span></p>` : ''}
            </div>
        </div>
    `).join('') : '<div class="section-content"><p>No evidence was logged with this report.</p></div>';
    ...
    return `
        ...
        <body>
            <div class="report-container">
                ...
                <p class="no-print" style="font-size: 12px; color: #777;"><strong>Generated By:</strong> AI Co-Parenting Reporter Tool | <strong>Date:</strong> ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}</p>
                ...
                <article class="section"><ul>${sourcesList || '<li>None provided</li>'}</ul></article>
